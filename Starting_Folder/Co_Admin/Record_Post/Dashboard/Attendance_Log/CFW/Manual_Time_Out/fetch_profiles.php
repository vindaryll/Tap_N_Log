...
$query = "
    SELECT 
        ep.cfw_id, ep.first_name, ep.last_name, ep.cfw_rfid, ep.cfw_img, 
        ea.date_att, ea.time_in, ea.cfw_attendance_id
    FROM cfw_profile ep
    INNER JOIN cfw_attendance ea 
        ON ep.cfw_id = ea.cfw_id 
...
if (!empty($search)) {
    ...
    $query .= " AND (ep.first_name LIKE '%$search%' OR ep.last_name LIKE '%$search%' OR ep.cfw_rfid LIKE '%$search%')";
}
                ...
                <div class='card'>
                    ...
                    <div class='card-footer d-flex'>
                        <button type='button' class='btn btn-danger mx-1 w-50 archive-btn' 
                            data-attendance-id='{$row['cfw_attendance_id']}'
                            data-profile-id='{$row['cfw_id']}'
