...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $query = "SELECT * FROM cfw_profile WHERE cfw_rfid = ? AND status = 'ACTIVE'";
        ...
        $query = "
            SELECT * FROM cfw_attendance 
            WHERE cfw_id = ? AND date_att = ? AND is_archived = FALSE";
        $stmt = $conn->prepare($query);
        ...
        $response = [
            'success' => true,
            'profile' => [
                'attendance_id' => $attendance['cfw_attendance_id'],
                'id' => $profile['cfw_id'],
