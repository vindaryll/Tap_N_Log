...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $rfidForModal = $row['cfw_rfid'] ?? '';

        // Display "None" for RFID in the card view
        $rfidDisplay = $row['cfw_rfid'] ?? 'None';
        ...
        $buttonHtml = $row['status'] === 'ACTIVE' ? "
            <div class='col-6'>
                <button type='button' class='btn btn-warning w-100 edit-btn'
                    ...
                    data-img='/TAPNLOG/Image/CFW/{$row['cfw_img']}'>
                    EDIT
                </button>
            </div>
            <div class='col-6'>
                <button type='button' class='btn btn-danger w-100 status-btn'
                    data-id='{$row['cfw_id']}' data-status='INACTIVE'>
                    DEACTIVATE
                </button>
            </div>
        " : "
            <div class='col-12'>
                <button type='button' class='btn btn-success w-100 status-btn'
                    data-id='{$row['cfw_id']}' data-status='ACTIVE'>
                    REACTIVATE
                </button>
            </div>
        ";
            ...
            <div class='col-xl-3 col-lg-4 col-md-6 mt-2 card-container'>
                <div class='card'>
                    <div class='profile-image-container'>
                        <img src='/TAPNLOG/Image/CFW/{$row['cfw_img']}' class='card-img-top' alt='Profile Image'>
                    </div>
                    <div class='card-body'>
