...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $query = $conn->prepare("SELECT first_name, last_name, ojt_rfid, ojt_img FROM ojt_profile WHERE ojt_id = ?");
        ...
        $currentRFID = $currentData['ojt_rfid'];
        $currentFirstName = $currentData['first_name'];
        $currentLastName = $currentData['last_name'];
        $currentImageName = $currentData['ojt_img'];
        $imageDirectory = $_SESSION['directory'] . '/Image/OJT/';
        $updates = [];
        $logDetails = [];

        // Build log header
        $updatedName = "$firstName $lastName"; // Reflect updated name
        $logHeader = "Update On the Job Trainee Profile\n\nId: $profileId\nName: $updatedName\n\n";
        ...
        if ($currentRFID !== $rfid) {
            ...
            $updates[] = "ojt_rfid = " . ($rfid ? "'$rfid'" : "NULL");
        }
        ...
        if ($croppedImage) {
            ...
            $updates[] = "ojt_img = '$uniqueName'";
            $logDetails[] = "Change Profile Picture";
        }

        // Update the database
        if (!empty($updates)) {
            $updateSQL = "UPDATE ojt_profile SET " . implode(", ", $updates) . " WHERE ojt_id = $profileId";
