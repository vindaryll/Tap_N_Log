...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql = "
            SELECT ea.employee_attendance_id, ea.employee_id, ea.date_att, ea.time_in, ea.time_out, 
                   ep.first_name, ep.last_name
            FROM employees_attendance ea
            JOIN employees_profile ep ON ea.employee_id = ep.employee_id
            WHERE ea.employee_attendance_id = ?";
        ...
        $profile_id = $row['employee_id']; // Now using profile_id
        ...
        $archive_query = "UPDATE employees_attendance SET is_archived = FALSE WHERE employee_attendance_id = ?";
