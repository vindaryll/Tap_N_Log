...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql = "
            SELECT ea.cfw_attendance_id, ea.cfw_id, ea.date_att, ea.time_in, ea.time_out, 
                   ep.first_name, ep.last_name
            FROM cfw_attendance ea
            JOIN cfw_profile ep ON ea.cfw_id = ep.cfw_id
            WHERE ea.cfw_attendance_id = ?";
        ...
        $profile_id = $row['cfw_id']; // Now using profile_id
        ...
        $archive_query = "UPDATE cfw_attendance SET is_archived = FALSE WHERE cfw_attendance_id = ?";
