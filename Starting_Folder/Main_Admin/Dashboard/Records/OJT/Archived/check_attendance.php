...
// Get ojt_id from attendance record
$query = "SELECT ojt_id, date_att FROM ojt_attendance WHERE ojt_attendance_id = ?";
...
if ($result->num_rows > 0) {
    ...
    // Check if there's any active attendance for this ojt for that day (is_archived = false)
    $checkQuery = "SELECT * FROM ojt_attendance WHERE ojt_id = ? AND date_att = ? AND is_archived = false LIMIT 1";
    $checkStmt = $conn->prepare($checkQuery);
    $checkStmt->bind_param("is", $ojt_id, $date);
    $checkStmt->execute();
